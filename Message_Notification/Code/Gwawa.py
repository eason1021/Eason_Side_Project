...
def Gwawa_Greet(stat, year, month, day):
    if stat == 'Morning':
        image = list(Path('/Users/yiming/Documents/Code_space/Gwawa/Image/Morning').glob('*.png'))
        rand_num = random.randint(0, len(image) - 1)
        message = '早安，我的好友。'
        image = image[rand_num]
        return message, image
    elif stat == 'Noon':
        image = list(Path('/Users/yiming/Documents/Code_space/Gwawa/Image/Noon').glob('*.png'))
        rand_num = random.randint(0, len(image) - 1)
        image = image[rand_num]
        message = '中午好，記得吃午餐哦!'
        return message, image
    elif stat == 'Night':
        image = list(Path('/Users/yiming/Documents/Code_space/Gwawa/Image/Night').glob('*.png'))
        rand_num = random.randint(0, len(image) - 1)
        image = image[rand_num]
        message = '晚上好，記得休息呦。'
        return message, image
    elif stat == 'Concern':
        image = list(Path('/Users/yiming/Documents/Code_space/Gwawa/Image/Concern').glob('*.png'))
        rand_num = random.randint(0, len(image) - 1)
        image = image[rand_num]
        message = '認同請分享'
        return message, image
        ...
        if month == 1 and day == 1:
            image = list(Path('/Users/yiming/Documents/Code_space/Gwawa/Image/NewYear').glob('*.png'))
            rand_num = random.randint(0, len(image) - 1)
            image = image[rand_num]
            message = '新年快樂'
            return message, image
        ...
        if date in HBD:
            image = list(Path('/Users/yiming/Documents/Code_space/Gwawa/Image/HBD').glob('*.png'))
            rand_num = random.randint(0, len(image) - 1)
            image = image[rand_num]
            message = '生日快樂'
            return message, image
